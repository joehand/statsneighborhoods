...
def adjust_rich_bin(df, col_suffix, inc_bin=200000):
    ...
    df['new_mid_pt' + col_suffix] = df['new_mid_pt' + col_suffix].replace([np.inf, -np.inf], inc_bin)
...
def calculate_mean_income(df, inc_bins, col_suffix):
    ...
    df['total_inc' + col_suffix] = df.filter(regex='^ACSHINC([0-9])+' + col_suffix).sum(axis=1)
    df['mean_inc' + col_suffix] = df['total_inc' + col_suffix]//df['Total_Households']
...
def calculate_a(df, reported_mean='ACSAVGHINC', calc_mean_low='mean_inc_low',
    ...
    df = df.copy()
    df['a'] = (df[reported_mean] - df[calc_mean_low])/(df[calc_mean_mid] - df[calc_mean_low])
    return df
